...
def _init_toolkit():
    ...
    for tk in toolkits:
        # Try and import the toolkit's enable2 backend.
        be = 'enthought.enable2.%s_backend.api' % tk
        ...
        if toolkit:
            raise ImportError, "unable to import an enable2 backend for the %s toolkit" % toolkit
        else:
            raise ImportError, "unable to import an enable2 backend for any of the %s toolkits" % ", ".join(known_toolkits)
...
def toolkit_object(name):
    ...
    try:
        ...
        raise NotImplementedError("the %s enable2 backend doesn't implement %s" % (ETSConfig.toolkit, name))
