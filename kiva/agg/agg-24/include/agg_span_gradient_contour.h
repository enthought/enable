...
namespace agg24
{
	...
	public:
		...
		int calculate(int x, int y, int d) const
		{
			if (m_buffer)
			{
				int px = x >> agg24::gradient_subpixel_shift;
	...
	int8u* gradient_contour::contour_create(path_storage* ps )
	{
		...
		if (ps)
		{
		   ...
		   // Path Bounding Box + Some Frame Space Around [configurable]
			agg24::conv_curve<agg24::path_storage> conv(*ps);

			double x1, y1, x2, y2;

			if (agg24::bounding_rect_single(conv ,0 ,&x1 ,&y1 ,&x2 ,&y2 ))
			{
			   // Create BW Rendering Surface
				...
				if (buffer)
				{
				   ...
				   // Setup VG Engine & Render
					agg24::rendering_buffer rb;
					rb.attach(buffer ,width ,height ,width );

					agg24::pixfmt_gray8 pf(rb);
					agg24::renderer_base<agg24::pixfmt_gray8> renb(pf );

					agg24::renderer_primitives<agg24::renderer_base<agg24::pixfmt_gray8> > prim(renb );
					agg24::rasterizer_outline<renderer_primitives<agg24::renderer_base<agg24::pixfmt_gray8> > > ras(prim );

					agg24::trans_affine mtx;
					mtx *= agg24::trans_affine_translation(-x1 + m_frame, -y1 + m_frame );

					agg24::conv_transform<agg24::conv_curve<agg24::path_storage> > trans(conv ,mtx );

					prim.line_color(agg24::rgba8(0 ,0 ,0 ,255 ) );
					ras.add_path(trans );
