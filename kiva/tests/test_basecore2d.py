...
from numpy import all, array, ravel
...
class GraphicsContextTestCase(unittest.TestCase):
    ...
    def test_state_fill_color(self):
        gc = basecore2d.GraphicsContextBase()
        # defaults to [0,0,0,1]
        self.assertTrue(all(gc.state.fill_color == array([0, 0, 0, 1])))
        gc.set_fill_color((0, 1, 0, 1))
        gc.save_state()
        gc.set_fill_color((1, 1, 1, 1))
        self.assertTrue(all(gc.state.fill_color == array([1, 1, 1, 1])))
        gc.restore_state()
        self.assertTrue(all(gc.state.fill_color == array([0, 1, 0, 1])))

    def test_state_stroke_color(self):
        gc = basecore2d.GraphicsContextBase()
        # defaults to [0,0,0,1]
        self.assertTrue(
            all(gc.state.line_state.line_color == array([0, 0, 0, 1]))
        )
        gc.set_stroke_color((0, 1, 0, 1))
        gc.save_state()
        gc.set_stroke_color((1, 1, 1, 1))
        self.assertTrue(
            all(gc.state.line_state.line_color == array([1, 1, 1, 1]))
        )
        gc.restore_state()
        self.assertTrue(
            all(gc.state.line_state.line_color == array([0, 1, 0, 1]))
        )
    ...
    def test_state_context_manager(self):
        ...
        with gc:
            ...
            gc.set_fill_color((1, 1, 1, 1))
            self.assertTrue(
                all(gc.state.fill_color == array([1, 1, 1, 1]))
            )
    ...
    def test_state_context_manager_nested(self):
        ...
        with gc:
            ...
            with gc:
                ...
                self.assertTrue(
                    all(gc.state.fill_color == array([1, 1, 0, 1]))
                )
