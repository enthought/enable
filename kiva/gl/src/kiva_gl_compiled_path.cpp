...
void
compiled_path::remove_all()
{
    //kiva_gl_agg::path_storage::remove_all();
    // fix me: call to base:: to appease VC++6.0
    this->base::remove_all();
    this->_has_curves = false;
    //ptm = kiva_gl_agg::trans_affine();
}

void
compiled_path::begin_path()
{
    this->remove_all();
}

void
compiled_path::close_path()
{
    this->close_polygon();
}

void
compiled_path::move_to(double x, double y)
{
...
void
compiled_path::line_to(double x, double y)
{
    this->ptm.transform(&x, &y);
    // fix me: call to base:: to appease VC++6.0
...
void
compiled_path::quad_curve_to(double x_ctrl, double y_ctrl,
...
void
compiled_path::curve_to(double x_ctrl1, double y_ctrl1,
...
void
compiled_path::arc(double x, double y, double radius, double start_angle,
                   double end_angle, bool cw)
{
    ...
    if (cw)
    {
        sweep_angle = -(2*kiva_gl_agg::pi - sweep_angle);
    }
    kiva_gl_agg::bezier_arc aggarc(x, y, radius, radius, start_angle, sweep_angle);
...
void
compiled_path::arc_to(double x1, double y1, double x2, double y2,
                      double radius)
{
    ...
    kiva_gl_agg::trans_affine_translation xform(-x1, -y1);
    double xform_angle = -atan2(y0-y1, x0-x1);
    if (!kiva_gl::almost_equal(fmod(xform_angle, 2*kiva_gl_agg::pi), 0.0))
    {
        xform *= kiva_gl_agg::trans_affine_rotation(xform_angle);
    }
    ...
    assert(kiva_gl::almost_equal(y1, 0.0));
    assert(kiva_gl::almost_equal(x1, 0.0));
    ...
    kiva_gl_agg::bezier_arc_svg aggarc(x0, y0, radius, radius, 0.0, false, sweep_flag, x2, y2);
...
void
compiled_path::add_path(compiled_path& other_path)
{
    container_type& vertices = this->vertices();
    double x=0.0;
    double y=0.0;
    ...
    while (!kiva_gl_agg::is_stop(cmd))
    {
        this->_has_curves |= kiva_gl_agg::is_curve(cmd);
        this->ptm.transform(&x, &y);
...
void
compiled_path::lines(double* pts, int Npts)
{
...
void
compiled_path::line_set(double* start, int Nstart, double* end, int Nend)
{
...
void
compiled_path::rect(double x, double y, double sx, double sy)
{
    this->move_to(x, y);
    this->line_to(x, y+sy);
...
void
compiled_path::rect(kiva_gl::rect_type &r)
{
    this->rect(r.x, r.y, r.w, r.h);
}

void
compiled_path::rects(double* all_rects, int Nrects)
{
    double *tmp;
...
void
compiled_path::rects(kiva_gl::rect_list_type &rectlist)
{
    for (kiva_gl::rect_list_type::iterator it=rectlist.begin(); it != rectlist.end(); ++it)
    {
...
void
compiled_path::_transform_ctm(kiva_gl_agg::trans_affine& m)
{
    this->ptm.premultiply(m);
}

void
compiled_path::translate_ctm(double x, double y)
{
    kiva_gl_agg::trans_affine_translation m(x, y);
    this->_transform_ctm(m);
}

void
compiled_path::rotate_ctm(double angle)
{
    kiva_gl_agg::trans_affine_rotation m(angle);
    this->_transform_ctm(m);
}

void
compiled_path::scale_ctm(double sx, double sy)
{
    kiva_gl_agg::trans_affine_scaling m(sx, sy);
    this->_transform_ctm(m);
}

void
compiled_path::concat_ctm(kiva_gl_agg::trans_affine& m)
{
    kiva_gl_agg::trans_affine m_copy(m);
    this->_transform_ctm(m_copy);
}

void
compiled_path::set_ctm(kiva_gl_agg::trans_affine& m)
{
    this->ptm = kiva_gl_agg::trans_affine(m);
}

kiva_gl_agg::trans_affine
compiled_path::get_ctm()
{
    return this->ptm;
}

void
compiled_path::save_ctm()
{
    this->ptm_stack.push(this->ptm);
}

void
compiled_path::restore_ctm()
{
