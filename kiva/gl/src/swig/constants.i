...
%{
    inline unsigned path_cmd(unsigned c) { return c & kiva_gl_agg::path_cmd_mask; }
    inline unsigned path_flags(unsigned c) { return c & kiva_gl_agg::path_flags_mask; }
%}

%include "agg_typemaps.i"

%apply(kiva_enum_typemap) { kiva_gl_agg::path_flags_e };
%apply(kiva_enum_typemap) { kiva_gl::marker_e };
%apply(kiva_enum_typemap) { kiva_gl::draw_mode_e mode, kiva_gl::line_join_e,
                            kiva_gl::line_cap_e, kiva_gl::blend_mode_e };
%apply(kiva_enum_typemap) { kiva_gl::pix_format_e, kiva_gl::interpolation_e };
%apply(kiva_enum_typemap) { kiva_gl::blend_mode_e mode};
