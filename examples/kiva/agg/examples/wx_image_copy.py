...
class LionCanvas(Canvas):
    ...
    def do_draw(self,gc):
        ...
        # just draw in a copy of the serengeti.
        gc.save_state()
        # fix me: need to blend modes set up as kiva constants
        from enthought.kiva.agg import blend_copy
        #gc.set_blend_mode(blend_copy)
        w,h = self.raw_serengeti.width(), self.raw_serengeti.height()
        #gc.draw_image(self.raw_serengeti,(100,100,w, h))
        
        #gc.set_blend_mode(blend_copy)
        gc.draw_image(self.raw_serengeti,(0, 0, w, h))
        gc.restore_state()
        ...
        print "image, overlay, blit, clear, total:", (self.image_time, 
                                               self.lion_time, 
                                               self.blit_time, 
                                               self.clear_time,                                  
                                               self.total_paint_time)
        ...
        text = "overlay time: %3.3f, %3.3f" % (self.lion_time, 1/(self.lion_time+.00001))
        gc.show_text(text,(10,sz[1] - 160))
        gc.restore_state()
